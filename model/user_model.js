...
async function checkUserExist(provider, email) {
    try {
...
async function insertNewOauthUser(provider, email, username) {
    const conn = await pool.getConnection();
    try {
        await conn.query('START TRANSACTION;');
        const [result] = await conn.query(
...
async function updateCoins(coins, id) {
    ...
    try {
        await conn.query('START TRANSACTION;');
        await conn.query('UPDATE user SET coins = coins + ? WHERE id = ?', [coins, id]);
        await conn.query('COMMIT');
        return true;
    } catch (err) {
        console.error(err);
...
async function updateCatClicked(cat_clicked, id) {
    ...
    try {
        await conn.query('START TRANSACTION;');
        await conn.query('UPDATE user SET cat_clicked = cat_clicked + ? WHERE id = ?', [
            cat_clicked,
            id,
        ]);
        await conn.query('COMMIT');
...
async function updateLikedCount(liked_counts, id) {
    ...
    try {
        await conn.query('START TRANSACTION;');
        await conn.query('UPDATE user SET liked_counts = liked_counts + ? WHERE id = ?', [
            liked_counts,
            id,
        ]);
        await conn.query('COMMIT');
...
async function updateLinkCounts(link_counts, id) {
    ...
    try {
        await conn.query('START TRANSACTION;');
        await conn.query('UPDATE user SET link_counts = link_counts + ? WHERE id = ?', [
            link_counts,
            id,
        ]);
        await conn.query('COMMIT');
...
async function updateMissionCompleted(mission_completed, id) {
    ...
    try {
        await conn.query('START TRANSACTION;');
        await conn.query('UPDATE user SET mission_completed = mission_completed + ? WHERE id = ?', [
            mission_completed,
            id,
        ]);
        await conn.query('COMMIT');
...
async function updateLoginDate(id) {
    const conn = await pool.getConnection();
    try {
        await conn.query('START TRANSACTION;');
        await conn.query('UPDATE user SET login_date = CURRENT_TIMESTAMP() WHERE id = ?', [id]);
        await conn.query('COMMIT');
        return true;
    } catch (err) {
        console.error(err);
