...
const sessionCheck = async (req, res, next) => {
    ...
    if (!userId) {
        ...
        return next(errRes(401, { status: 2006, msg: 'session error!' }));
    }
    const userData = await cache.get(`user:${userId}`);
    req.body.userData = JSON.parse(userData);
    return next();
...
const checkUser = async (req, res) => {
    return res.status(200).json({ data: 'session checked' });
};
...
module.exports = { reCaptcha, sessionCheck, sessionSoftCheck, postUserSignUp, postUserSignIn, getUser, logoutUser, checkUser };
