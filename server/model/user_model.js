...
async function insertNewOauthUser(provider, email, username) {
    ...
    try {
        await conn.query('START TRANSACTION;');
        const [result] = await conn.query('INSERT INTO user(provider, email, username) VALUES (?,?,?)', [provider, email, username]);
        await conn.query('COMMIT');
        return result.insertId;
    } catch (err) {
        console.error(err);
...
async function updateLoginDate(id) {
    ...
    try {
        await conn.query('START TRANSACTION;');
        const [result] = await conn.query('UPDATE user SET login_date = CURRENT_TIMESTAMP() WHERE id = ?', [id]);
        await conn.query('COMMIT');
        return true;
    } catch (err) {
        console.error(err);
